...
export class ProductsService {
  ...
  search(query: string, params?: { category?: string; topPick?: boolean; hotDeal?: boolean; bestSelling?: boolean }) {
    let httpParams = new HttpParams();
    ...
    if (params?.hotDeal) httpParams = httpParams.set("hotDeal", "true");
    if (params?.bestSelling) httpParams = httpParams.set("bestSelling", "true");
    return this.http.get<Product[]>(`${environment.apiBaseUrl}/api/products/search`, { params: httpParams });
  }
